...
- **メモ機能**: ⌘N（アプリ内）/ ⌘⇧N（グローバル）でテキストメモを即時追加・インライン編集可能
- **HTTP API**: ローカル HTTP サーバーが Push 受信と現在アイテムの GET 取得に対応
...
| **メモ** | ⌘N / ⌘⇧N でテキストメモをダッシュボードに即時追加。カード上でインライン編集も可能 |
| **HTTP API** | `GET /items` で現在のダッシュボードアイテムを JSON 取得（`?exclude_images=true` で画像除外可能） |
...
| `tauri-plugin-clipboard-manager` | Tauri クリップボードプラグイン（テキスト読み書き） |
| `tauri-plugin-global-shortcut` | OS レベルのグローバルショートカット登録 |
| `tauri-plugin-dialog` | ネイティブファイル保存ダイアログ |
...
│   ├── App.tsx                   # ルートコンポーネント
│   ├── components/
│   │   ├── Dashboard.tsx         # メインダッシュボード（フィルタ・一覧・エクスポート）
│   │   ├── ItemCard.tsx          # アイテムカード（日時・タイトル・タグ・ブックマーク・インライン編集）
│   │   ├── NoteInput.tsx         # メモ入力モーダル（⌘N / ⌘⇧N で開く）
...
アプリ起動
  ...
  └─ Push サーバー起動 (axum, 127.0.0.1:8080)
      └─ POST /push: 外部からのプッシュ受信
      └─ GET  /items: 全アイテム JSON 取得（?exclude_images=true で画像省略可）
  └─ bookmarks.json 読み込み → ストアへ追加（source_id = "bookmark"）
  └─ クリップボード監視起動（poll_interval_secs 間隔）
  └─ グローバルショートカット登録（⌘⇧N → メモ入力ウィンドウを開く）
```
...
```
⌘N（アプリ内）または ⌘⇧N（グローバルショートカット）
  └─ NoteInput モーダルが開く（アプリがバックグラウンドでも動作）
  └─ テキスト入力 → add_note コマンド
      └─ source_id = "note"、source_name = "メモ" として DashItem を作成
      └─ 1行目をタイトル、全文を body に設定
      └─ published_at に現在日時を記録
      └─ DashStore に push → dashboard_updated イベント emit

✏️ ボタンクリック（メモカードのみ表示）
  └─ カードをインライン編集モードに切り替え
  └─ ⌘+Enter で保存 / Esc でキャンセル
  └─ edit_note コマンド → ストア内のアイテムを更新
```
...
| Method | Path | 認証 | 説明 |
|--------|------|------|------|
| `GET` | `/health` | 不要 | サーバー疎通確認 |
| `POST` | `/push` | Bearer | アイテムのプッシュ受信 |
| `GET` | `/items` | Bearer | 現在のダッシュボードアイテムを JSON で取得 |

### 認証

- `Authorization: Bearer <token>` ヘッダー必須（`/health` を除く）
...
現在ダッシュボードに表示されているアイテムをすべて JSON で返す。

**クエリパラメータ:**

| パラメータ | 値 | 説明 |
|-----------|-----|------|
| `exclude_images` | `true` | `image_data` フィールド（Base64 PNG）をレスポンスから除外する |
  ...
  "items": [
    {
      ...
      "url": "https://zenn.dev/...",
      "image_data": null,
      "published_at": "2026-03-15T09:00:00+09:00",
      "tags": []
    }
  ]
}
...
| Command | `add_note` | Front → Back | テキストメモをダッシュボードに追加 |
| Command | `edit_note` | Front → Back | 既存メモのテキストを更新 |
| Command | `export_items_to_path` | Front → Back | ネイティブ保存ダイアログでパスを選択して JSON エクスポート |
| Command | `get_items_json` | Front → Back | 全アイテムの JSON 文字列を取得（クリップボードコピー用） |
| Event | `dashboard_updated` | Back → Front | 新規アイテム追加・変更通知 |
| Event | `open_note_input` | Back → Front | グローバルショートカット（⌘⇧N）によるメモ入力モーダル開示 |
...
<App>
  └── <Dashboard>
        ├── <NoteInput>    メモ入力モーダル（⌘N / ⌘⇧N で開く、Esc で閉じる）
        ├── <header>       アイテム件数・✏️メモ追加・📋JSON コピー・↓ JSON 保存・↻ 更新
        ...
        └── <ItemCard>     各アイテムカード
              ...
              ├── タイトル（クリックでブラウザ起動）
              ├── 画像（クリップボード画像の場合）
              ├── 本文サマリ（3 行クランプ）または インライン編集テキストエリア
...
| ショートカット | 動作 |
|--------------|------|
| `⌘N` / `Ctrl+N` | メモ入力モーダルを開く（アプリフォーカス時） |
| `⌘⇧N` / `Ctrl+Shift+N` | メモ入力モーダルを開く（グローバル・バックグラウンド時も有効） |
| `⌘↵` / `Ctrl+Enter` | メモを保存（モーダル内・カード編集中） |
| `Esc` | モーダルを閉じる / 編集をキャンセル |
...
# 画像データを除いて取得（レスポンス軽量化）
curl -H "Authorization: Bearer $TOKEN" \
  "http://localhost:$PORT/items?exclude_images=true"
```
...
| **JSON エクスポート** | ネイティブ保存ダイアログでファイルパスを選択して JSON を書き出す | ✅ 実装済み |
| **クリップボード JSON コピー** | 現在のアイテムを JSON 形式でクリップボードに直接コピー | ✅ 実装済み |
| **メモ機能** | ⌘N / ⌘⇧N でテキストメモをダッシュボードに追加・インライン編集 | ✅ 実装済み |
| **グローバルショートカット** | バックグラウンド時も ⌘⇧N でメモ入力ウィンドウを起動 | ✅ 実装済み |
| **GET /items API** | HTTP サーバーで現在アイテムを取得（`?exclude_images=true` 対応） | ✅ 実装済み |
