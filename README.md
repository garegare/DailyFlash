...
| **通知バッジ** | タスクトレイアイコンに未読件数バッジを表示 |
| **複数ウィンドウ対応** | ソース別に独立したウィンドウで表示 |
...
本プロジェクトが使用するすべてのライブラリ（Rust クレート・npm パッケージ）は MIT または Apache-2.0 ライセンスです。コピーレフト（GPL 等）ライセンスのライブラリは含まれていないため、MIT ライセンスでの公開に問題はありません。
